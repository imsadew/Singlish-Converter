# Real-time Singlish to Sinhala Converter

A simple web tool, similar to SAP Singlish Converter, that converts Singlish characters to Sinhala Unicode in real-time as you type.

**Live Demo:** [#](#)

## ✨ Features

* Real-time Singlish to Sinhala Unicode conversion.
* Ability to copy the converted text.
* Export the converted text as a `.txt` file.
* Import text from a `.txt` file.
* Share text via the native Share menu.
* Light/Dark mode theme toggle.
* Responsive design adaptable to any screen size.

## 🚀 How to Use

1.  Go to the [Live Demo](#).
2.  Start typing Singlish in the left input box.
3.  The Sinhala Unicode text will appear instantly in the right output box.
4.  Use the buttons below the output box (Copy, Export, Import, Share) for actions.

## 🛠️ Built With

* HTML
* CSS (Tailwind CSS - for the new UI)
* JavaScript

--------------------------------------------
